...
public class SingletonMySqlContainer {
  ...
  private static SingletonMySqlContainer INSTANCE;
  ...
  private SingletonMySqlContainer() {
    ...
    container =
        new MySQLContainer<>(IMAGE_VERSION)
            .withUsername("test")
            .withPassword("test")
  ...
  public static SingletonMySqlContainer getInstance() {
    if (INSTANCE == null) {
      INSTANCE = new SingletonMySqlContainer();
    }
    return INSTANCE;
  }
  ...
  public void start() {
    ...
    String baseUrl = "jdbc:mysql://%s:%s//test-db?useSSL=false&requireSSL=false&serverTimezone=UTC";
    ...
    if (System.getenv("SKIP_MYSQL_CONTAINER") == null) {
      container.start();
