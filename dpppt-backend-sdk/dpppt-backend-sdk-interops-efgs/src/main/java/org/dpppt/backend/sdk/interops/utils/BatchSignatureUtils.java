...
import org.dpppt.backend.sdk.interops.model.EfgsProto.DiagnosisKey;
import org.dpppt.backend.sdk.interops.model.EfgsProto.DiagnosisKeyBatch;
...
public class BatchSignatureUtils {
   ...
   */
  static byte[] generateBytes(final DiagnosisKeyBatch batch) {
    final ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();

    batch.getKeysList().stream()
      .map(BatchSignatureUtils::generateBytes)
   ...
   */
  public static byte[] generateBytes(final DiagnosisKey diagnosisKey) {
    final ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
  ...
  private static List<DiagnosisKey> sortBatchByKeyData(DiagnosisKeyBatch batch) {
    return batch.getKeysList()
      .stream()
      .sorted(Comparator.nullsLast(
        Comparator.comparing(diagnosisKey -> bytesToBase64(generateBytes(diagnosisKey)))
      ))
